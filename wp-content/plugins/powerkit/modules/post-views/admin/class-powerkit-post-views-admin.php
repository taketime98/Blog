...
class Powerkit_Post_Views_Admin extends Powerkit_Module_Admin {
							...
							<tbody>
								...
								if ( empty( $options['token'] ) ) {
										...
										?>
											<tr>
												<td colspan="2">
													...
													<p><a class="button" href="<?php echo esc_url( powerkit_get_page_url( $this->slug . '&state=clear-api' ) ); ?>"><?php esc_html_e( 'Clear the API keys', 'powerkit' ); ?> &raquo;</a></p>
												</td>
											</tr>
									...
									?>
										...
										<p><a href="<?php echo esc_url( powerkit_get_page_url( $this->slug . '&state=disconnect' ) ); ?>"><?php esc_html_e( 'Disconnect from Google Analytics', 'powerkit' ); ?> &raquo;</a></p>

										<p><a href="<?php echo esc_url( powerkit_get_page_url( $this->slug . '&state=reset-cache' ) ); ?>"><?php esc_html_e( 'Empty page views cache', 'powerkit' ); ?> &raquo;</a></p>
	...
	public function process_options_page() {
		...
		if ( isset( $_REQUEST['save_settings'] ) ) { // Input var ok; sanitization ok.

			if ( isset( $_REQUEST['powerkit_post_views_clientid'] ) ) { // Input var ok; sanitization ok.
				$options['clientid'] = sanitize_text_field( $_REQUEST['powerkit_post_views_clientid'] ); // Input var ok; sanitization ok.
			}

			if ( isset( $_REQUEST['powerkit_post_views_psecret'] ) ) { // Input var ok; sanitization ok.
				$options['psecret'] = sanitize_text_field( $_REQUEST['powerkit_post_views_psecret'] ); // Input var ok; sanitization ok.
			}

			if ( isset( $_REQUEST['powerkit_post_views_wid'] ) ) { // Input var ok; sanitization ok.
				$options['wid'] = sanitize_text_field( $_REQUEST['powerkit_post_views_wid'] ); // Input var ok; sanitization ok.
			}

			if ( isset( $_REQUEST['powerkit_post_views_startdate'] ) ) { // Input var ok; sanitization ok.
				$options['startdate'] = sanitize_text_field( $_REQUEST['powerkit_post_views_startdate'] ); // Input var ok; sanitization ok.
			}

			if ( isset( $_REQUEST['powerkit_post_views_defaultval'] ) ) { // Input var ok; sanitization ok.
				$options['defaultval'] = sanitize_text_field( $_REQUEST['powerkit_post_views_defaultval'] ); // Input var ok; sanitization ok.
			}

			if ( isset( $_REQUEST['powerkit_post_views_metric'] ) ) { // Input var ok; sanitization ok.
				$options['metric'] = sanitize_text_field( $_REQUEST['powerkit_post_views_metric'] ); // Input var ok; sanitization ok.
			}

			$options['column']   = ( isset( $_REQUEST['powerkit_post_views_column'] ) ); // Input var ok; sanitization ok.
			$options['trailing'] = ( isset( $_REQUEST['powerkit_post_views_trailing'] ) ); // Input var ok; sanitization ok.
