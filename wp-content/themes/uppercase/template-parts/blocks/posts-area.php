...
if ( $posts->have_posts() ) {
	...
	// Posts Layout.
	$main_classes = ' cs-posts-area__' . esc_attr( $attributes['layout'] );
			...
			if ( 'standard' === $pagination_type ) {
				...
				if ( $posts->max_num_pages > 1 ) {
					...
					if ( $base_url ) {
						...
						cnvs_alert_warning( esc_html__( 'Please select a page for your blog posts in Settings &rarr; Reading to display pagination.', 'uppercase' ) );
					}
	...
	cnvs_alert_warning( esc_html__( 'There aren\'t enough posts that match the filter criteria.', 'uppercase' ) );
}
