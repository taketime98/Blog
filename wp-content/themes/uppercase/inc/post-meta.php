	...
	function csco_get_meta_author( $tag = 'div', $compact = true, $settings = array() ) {
		...
		if ( $authors ) {
			...
			foreach ( $authors as & $author ) {
				...
				$output .= $counter > 0 ? sprintf( '<span class="cs-sep">%s</span>', esc_html__( 'and', 'uppercase' ) ) : '';
				...
				$output .= sprintf(
					...
					esc_attr( sprintf( __( 'View all posts by %s', 'uppercase' ), $display_name ) ),
					$display_name,
					$author_avatar
				);
...
if ( ! function_exists( 'csco_get_meta_comments' ) ) {
	...
	function csco_get_meta_comments( $tag = 'div', $compact = false, $settings = array() ) {
		...
		if ( true === $compact ) {
			...
			ob_start();
			comments_popup_link( esc_html__( 'No comments', 'uppercase' ), esc_html__( 'One comment', 'uppercase' ), '% ' . esc_html__( 'comments', 'uppercase' ), 'comments-link', '' );
			$output .= ob_get_clean();
		}
...
if ( ! function_exists( 'csco_get_meta_reading_time' ) ) {
	...
	function csco_get_meta_reading_time( $tag = 'div', $compact = false, $settings = array() ) {
		...
		if ( true === $compact ) {
			$output .= intval( $reading_time ) . ' ' . esc_html__( 'min', 'uppercase' );
		} else {
			/* translators: %s number of minutes */
			$output .= esc_html( sprintf( _n( '%s minute read', '%s minute read', $reading_time, 'uppercase' ), $reading_time ) );
		}
...
if ( ! function_exists( 'csco_get_meta_views' ) ) {
	...
	function csco_get_meta_views( $tag = 'div', $compact = false, $settings = array() ) {
		...
		if ( true === $compact ) {
			...
			if ( $views > 1000 ) {
				$output .= $views_rounded . ' ' . esc_html__( 'views', 'uppercase' );
			} else {
				/* translators: %s number of post views */
				$output .= esc_html( sprintf( _n( '%s view', '%s views', $views, 'uppercase' ), $views ) );
			}
		}
			...
			<div class="cs-meta-share-total">
				...
				<span class="cs-total-number">
					...
					if ( true === $compact ) {
						...
						if ( $shares > 1000 ) {
							echo esc_html( $shares_rounded ) . ' ' . esc_html__( 'shares', 'uppercase' );
						} else {
							/* translators: %s number of post views */
							echo esc_html( sprintf( _n( '%s share', '%s shares', $shares, 'uppercase' ), $shares ) );
						}
					}
					?>
				</span>
