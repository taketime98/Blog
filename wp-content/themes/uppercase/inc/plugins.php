...
function csco_plugin_set_options( $plugin ) {
	...
	if ( 'rank-math' === $plugin ) {
		// Get display options.
		$display_options = get_option( 'rank-math-options-general' );
		// Set default value.
		$display_options['breadcrumbs']                   = 'on';
		$display_options['breadcrumbs_separator']         = '';
		$display_options['breadcrumbs_remove_post_title'] = 'on';
		// Update options.
		update_option( 'rank-math-options-general', $display_options );
	}
}
...
function csco_activated_plugin( $plugin ) {
	...
	// Check if plugin Rank Math Seo activated.
	if ( 'seo-by-rank-math/rank-math.php' === $plugin ) {
		csco_plugin_set_options( 'rank-math' );
	}
}
