...
function csco_get_load_more_args( $data, $attributes = false, $options = false ) {
	...
	$args = array(
		...
		'translation'    => array(
			'load_more' => esc_html__( 'Load More', 'uppercase' ),
			'loading'   => esc_html__( 'Loading', 'uppercase' ),
		),
	);
	...
	if ( $the_query->have_posts() ) :
		...
		while ( $the_query->have_posts() ) :
			...
			if ( isset( $response['attributes'] ) && $response['attributes'] ) {
				...
				if ( 'tile-two' === $attributes['layout'] || 'tile-mixed' === $attributes['layout'] ) {
					$attributes['layout'] = 'tile';
				}

				$template = $attributes['layout'];

				get_template_part( 'template-parts/blocks/posts-area/' . $template );
				...
				if ( 'tile-two' === $options['layout'] || 'tile-mixed' === $options['layout'] ) {
					$options['layout'] = 'tile';
				}

				$template = $options['layout'];
				if ( 'mosaic-two' === $options['layout'] ) {
					$template = 'list-fullwidth';
				}

				if ( 'full-fullwidth' === $options['layout'] ) {
					$template = 'full';
				}
