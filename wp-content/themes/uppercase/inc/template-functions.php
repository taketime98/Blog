...
if ( ! function_exists( 'csco_get_round_number' ) ) {
	...
	function csco_get_round_number( $number, $min_value = 1000, $decimal = 1 ) {
		...
		$alphabets = array(
			1000000000 => esc_html__( 'B', 'uppercase' ),
			1000000    => esc_html__( 'M', 'uppercase' ),
			1000       => esc_html__( 'K', 'uppercase' ),
		);
...
if ( ! function_exists( 'csco_hero_scheme' ) ) {
	/**
	 * Hero Data Scheme Attribute
	 */
	function csco_hero_scheme() {

		$scheme_data = csco_site_scheme_data();
		...
		// Set inverse scheme.
		if ( csco_has_hero_background() || csco_get_state_load_nextpost() ) {
			$scheme = 'inverse';
			$force  = 'true';
		}

		// Apply filters for hero scheme.
		$scheme = apply_filters( 'csco_hero_scheme', $scheme );

		$scheme_attr = sprintf( 'data-scheme="%s" data-force="%s"', $scheme, $force );

		echo wp_kses_post( $scheme_attr );
	}
}

if ( ! function_exists( 'csco_post_item_scheme' ) ) {
	/**
	 * Post Item Data Scheme Attribute
	 */
	function csco_post_item_scheme() {

		$scheme = 'inverse';

		// Apply filters for hero scheme.
		$scheme = apply_filters( 'csco_post_item_scheme', $scheme );

		$scheme_attr = 'data-scheme="' . $scheme . '"';
...
if ( ! function_exists( 'csco_section_heading' ) ) {
	...
	function csco_section_heading( $title, $type = 'full', $echo = true, $class = '', $location = 'default' ) {
		...
		if ( 'full' === $type || 'before' === $type ) {
			echo '<' . esc_html( $tag ) . ' class="cs-section-heading' . esc_attr( $class ) . '">';
		}
...
if ( ! function_exists( 'csco_get_video_markup' ) ) {
	/**
	 * Retrieve the markup for video
	 *
	 * @param string       $video_url Youtube video url.
	 * @param string|array $args {
	 *     Custom options.
	 *     @type bool   $controls Display tools.
	 *     @type bool   $link Display link.
	 *     @type string $start Video start time.
	 *     @type string $end Video end time.
	 *     @type string $template Template.
	 * }
	 *
	 * @return string The markup for video on success.
	 */
	function csco_get_video_markup( $video_url = null, $args = '' ) {
		...
		$id = csco_get_youtube_video_id( $video_url );
		if ( $id ) {
			ob_start();
			?>
			<div class="cs-video-wrapper" data-video-id="<?php echo esc_attr( $id ); ?>" data-video-start="<?php echo esc_attr( $args['start'] ); ?>" data-video-end="<?php echo esc_attr( $args['end'] ); ?>">
				<div class="cs-video-inner"></div>
				<div class="cs-video-loader"></div>
			</div>

			<?php if ( $args['controls'] ) { ?>
				<div class="cs-video-controls cs-video-controls-<?php echo esc_attr( $args['template'] ); ?>">
					<?php if ( $args['link'] ) { ?>
						<a class="cs-player-control cs-player-link cs-player-stop" target="_blank" href="<?php echo esc_url($video_url ); ?>">
							<span class="cs-tooltip"><span><?php esc_html_e( 'View on YouTube', 'uppercase' ); ?></span></span>
						</a>
					<?php } ?>
		...
		return apply_filters( 'csco_get_video_markup', $markup, $args );
	}
}

if ( ! function_exists( 'csco_the_video_markup' ) ) {
	/**
	 * Print the markup for video.
	 *
	 * @param string       $video_url Youtube video url.
	 * @param string|array $args {
	 *     Custom options.
	 ...
	 * }
	 */
	function csco_the_video_markup( $video_url = null, $args = '' ) {
		$video_markup = csco_get_video_markup( $video_url, $args );
		if ( empty( $video_markup ) ) {
			return;
		}

		echo wp_kses_post( $video_markup );
	}
}
...
if ( ! function_exists( 'csco_get_post_video' ) ) {
	 ...
	 * @param string|array $args Custom options.
	 */
	function csco_get_post_video( $post_id = null, $args = '' ) {

		if ( is_archive() || is_home() ) {
			$support = (array) get_post_meta( get_the_ID(), 'csco_post_video_location', true );
			if ( ! in_array( 'archive', $support, true ) ) {
				return;
			}
		}
		...
		$is_post = get_post_meta( $post_id );

		if ( $is_post ) {
			$video_url     = get_post_meta( $post_id, 'csco_post_video_url', true );
			$args['start'] = get_post_meta( $post_id, 'csco_post_video_bg_start_time', true );
			$args['end']   = get_post_meta( $post_id, 'csco_post_video_bg_end_time', true );
		}
...
if ( ! function_exists( 'csco_the_post_video' ) ) {
	/**
	 * Print post video.
	 *
	 * @param string       $post_id Post id.
	 * @param string|array $args Custom options.
	 */
	function csco_the_post_video( $post_id = null, $args = '' ) {

		$video_markup = csco_get_post_video( $post_id, $args );

		echo wp_kses_post( $video_markup );
	}
}

if ( ! function_exists( 'csco_get_hero_video' ) ) {
	/**
	 * Get hero video
	 */
	function csco_get_hero_video() {

		if ( is_singular() ) {
			$support = (array) get_post_meta( get_the_ID(), 'csco_post_video_location', true );
			if ( ! in_array( 'header', $support, true ) ) {
				return;
			}
		}
		...
		$video_markup = '';
		if ( is_singular() ) {
			$video_markup = csco_get_post_video();
		} elseif ( is_front_page() ) {
			$video_url = '';
			$args      = array();
			if ( get_theme_mod( 'hero_video' ) ) {
				$video_url = get_theme_mod( 'hero_video' );
			}
			if ( get_theme_mod( 'hero_video_start' ) ) {
				$args['start'] = get_theme_mod( 'hero_video_start' );
			}
			if ( get_theme_mod( 'hero_video_end' ) ) {
				$args['end'] = get_theme_mod( 'hero_video_end' );
			}
			if ( get_theme_mod( 'hero_video_controls', true ) ) {
				$args['controls'] = true;
			}
			$video_markup = csco_get_video_markup( $video_url, $args );
		}
...
if ( ! function_exists( 'csco_the_hero_video' ) ) {
	/**
	 * Print Header Video
	 */
	function csco_the_hero_video() {
		echo wp_kses_post( csco_get_hero_video() );
	}
}


if ( ! function_exists( 'csco_has_hero_video' ) ) {
	/**
	 * Check if hero has video
	 */
	function csco_has_hero_video() {
		return (bool) csco_get_hero_video();
	}
}
...
if ( ! function_exists( 'csco_get_hero_image_id' ) ) {
	/**
	 * Get Hero Image Id
	 */
	function csco_get_hero_image_id() {
		...
		if ( is_singular() ) {
			if ( get_post_thumbnail_id() ) {
				...
				if ( get_theme_mod( 'hero_background_image' ) ) {
					$image_id = get_theme_mod( 'hero_background_image' );
				}
			}
		} elseif ( is_category() ) {
			$term_id  = get_queried_object_id();
			$image_id = get_term_meta( $term_id, 'powerkit_featured_image', true );
		} elseif ( is_front_page() || is_search() ) {
			if ( get_theme_mod( 'hero_background_image' ) ) {
				$image_id = get_theme_mod( 'hero_background_image' );
			}
		} elseif ( is_home() && ! is_front_page() ) {
			$page_for_posts = get_option( 'page_for_posts' );
			if ( get_post_thumbnail_id( $page_for_posts ) ) {
				$image_id = get_post_thumbnail_id( $page_for_posts );
			}
		}

		return apply_filters( 'csco_hero_image_id', $image_id );

	}
}

if ( ! function_exists( 'csco_get_hero_image' ) ) {
	/**
	 * Get Hero Image
	 */
	function csco_get_hero_image() {
		$image_id = csco_get_hero_image_id();

		$size = get_theme_mod( 'hero_image_size', 'csco-hero' );
...
if ( ! function_exists( 'csco_the_hero_image' ) ) {
	/**
	 * Get Hero Image
	 */
	function csco_the_hero_image() {
		$image = csco_get_hero_image();

		echo wp_kses_post( $image );
	}
}

if ( ! function_exists( 'csco_has_hero_image' ) ) {
	/**
	 * Check If Hero Has Image
	 */
	function csco_has_hero_image() {
		$image_id     = csco_get_hero_image_id();
		$image_exists = wp_get_attachment_image_url( $image_id );

		return (bool) $image_exists;
	}
}

if ( ! function_exists( 'csco_has_hero_background' ) ) {
	/**
	 * Check If Hero Has Background
	 */
	function csco_has_hero_background() {
		if ( csco_has_hero_image() || csco_has_hero_video() ) {
			return true;
		}
	}
}

if ( ! function_exists( 'csco_is_content_fullwidth' ) ) {
	/**
	 * Check If Content Area Is Full Width
	 */
	function csco_is_content_fullwidth() {
		...
		if ( is_archive() || is_home() || is_search() ) {
			...
			if ( isset( $options['layout'] ) ) {
				...
				if ( in_array( $layout, $layout_fullwidth, true ) ) {
					return true;
				}
			}
		}
...
if ( ! function_exists( 'csco_get_archive_options' ) ) {
	...
	function csco_get_archive_options() {
		...
		$options = array(
			...
			'large_meta'                   => csco_get_archive_option( 'large_post_meta' ),
			...
			'tile_image_orientation'       => get_theme_mod( csco_get_archive_option( 'tile_image_orientation' ), 'landscape-16-9' ),
			'tile_large_image_orientation' => get_theme_mod( csco_get_archive_option( 'tile_large_image_orientation' ), 'landscape-16-9' ),
			'image_size'                   => get_theme_mod( csco_get_archive_option( 'image_size' ), 'csco-thumbnail' ),
			'image_size_full'              => get_theme_mod( csco_get_archive_option( 'image_size_full' ), 'csco-large' ),
			...
			'video_backgrounds'            => get_theme_mod( csco_get_archive_option( 'video_backgrounds' ), true ),
			'excerpt'                      => get_theme_mod( csco_get_archive_option( 'excerpt' ), true ),
			'large_excerpt'                => get_theme_mod( csco_get_archive_option( 'large_excerpt' ), true ),
...
if ( ! function_exists( 'csco_kirki_dynamic_css' ) ) {
	/**
	 * Add kirki dynamic css
	 */
	function csco_kirki_dynamic_css() {

		$layout = csco_get_archive_option( 'layout' );

		$styles = '';

		if ( 'tile' === get_theme_mod( $layout ) ) {
			$styles .= '
		...
		if ( 'tile-two' === get_theme_mod( $layout ) ) {
			$styles .= '
			.cs-posts-area__main.cs-posts-area__grid {
		...
		if ( 'tile-mixed' === get_theme_mod( $layout ) ) {
			$styles .= '
			.cs-posts-area__main.cs-posts-area__grid {
		...
		if ( csco_is_content_fullwidth() ) {
			$styles .= '.cs-posts-area__main{--cs-posts-area-grid-gap:0;}';
		}
